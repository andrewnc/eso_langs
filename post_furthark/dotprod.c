...
static void futrts_cli_entry_main(struct futhark_context *ctx)
{
    ...
    struct futhark_i32_1d *read_value_4025;
    int64_t read_shape_4026[1];
    int32_t *read_arr_4027 = NULL;
    
    errno = 0;
    if (read_array(&i32_info, (void **) &read_arr_4027, read_shape_4026, 1) !=
        0)
        panic(1, "Cannot read input #%d of type %s%s (errno: %s).\n", 0, "[]",
              i32_info.type_name, strerror(errno));
    
    struct futhark_i32_1d *read_value_4028;
    int64_t read_shape_4029[1];
    int32_t *read_arr_4030 = NULL;
    
    errno = 0;
    if (read_array(&i32_info, (void **) &read_arr_4030, read_shape_4029, 1) !=
        0)
        panic(1, "Cannot read input #%d of type %s%s (errno: %s).\n", 1, "[]",
              i32_info.type_name, strerror(errno));
    ...
    if (perform_warmup) {
        ...
        assert((read_value_4025 = futhark_new_i32_1d(ctx, read_arr_4027,
                                                     read_shape_4026[0])) != 0);
        assert((read_value_4028 = futhark_new_i32_1d(ctx, read_arr_4030,
                                                     read_shape_4029[0])) != 0);
        ...
        r = futhark_entry_main(ctx, &result_4031, read_value_4025,
                               read_value_4028);
        if (r != 0)
        ...
        assert(futhark_free_i32_1d(ctx, read_value_4025) == 0);
        assert(futhark_free_i32_1d(ctx, read_value_4028) == 0);
        ;
    }
    time_runs = 1;
    /* Proper run. */
    ...
    write_scalar(stdout, binary_output, &i32_info, &result_4031);
    printf("\n");
    ;
}
